...
vec4 snoiseD(vec3 v) {
  ...
  vec3 grad = -8.0 * (m3.x * pdotx.x * x0 + m3.y * pdotx.y * x1 + m3.z * pdotx.z * x2 + m3.w * pdotx.w * x3)
            + (m4.x * g0 + m4.y * g1 + m4.z * g2 + m4.w * g3);
  grad *= 42.0;
  return vec4(value, grad);
}

vec4 liquidDispWithGrad(vec3 p) {
  vec4 n1 = snoiseD(p * 0.55 + vec3(0.0, uTime * 0.12, 0.0));
  vec4 n2 = snoiseD(p * 1.10 + vec3(uTime * 0.08, 0.0, uTime * 0.06));
  float value = (n1.x + n2.x * 0.28) * 0.22;
  vec3 grad = (n1.yzw * 0.55 + n2.yzw * 1.10 * 0.28) * 0.22;
  return vec4(value, grad);
}
...
void main() {
  ...
  vec4 dispData = liquidDispWithGrad(pos);
  float disp = dispData.x;
  vec3 dispGrad = dispData.yzw;
  ...
  vec3 tanGrad = dispGrad - dot(dispGrad, normal) * normal;
  vec3 newNormal = normalize(normal - tanGrad);
...
const ORB_DRIFT_TIME_SCALE = 0.28;
const ORB_SURFACE_TIME_SCALE = 0.34;
const ORB_ROTATION_TIME_SCALE = 0.32;
